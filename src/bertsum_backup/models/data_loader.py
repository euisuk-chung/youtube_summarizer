...
class TextLoader:
    ...
    def load_text(self, source, delimiter=None):
        ...
        def _process_src(raw):
            ...
            if delimiter is not None:
                src_subtokens = [['[CLS]'] + self.tokenizer(r) + ['[SEP]'] for r in raw.split(delimiter)]
                raw_original = raw.split(delimiter)
            ...
            mask_src = (1 - (src == 1)).to(self.device)
        ...
        for x in source:
            x, x_original, src, mask_src, segments_ids, clss, mask_cls = _process_src(x)
            ...
            batch.mask_tgt  = None
            batch.segs  = segs
            batch.src_str  =  [[sent.replace('[SEP]','').strip().split() for sent in x.split('[CLS]')[1:]]]
            batch.src_ori = x_original
            batch.tgt_str  = ['']
            batch.clss  = clss
